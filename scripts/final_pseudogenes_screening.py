...
def untangle_paf(untangle_paf_file):
    ...
    # read in the paf file -this file must be "filter_passed_features.csv" coming from core_dispensable_pseudogenes.py
...
def untangle_paf_attributes(pangenome_screening_file):
    ...
    # read in the pangenome screening file - this file must be "pangenome_screening.csv" coming from core_dispensable_pseudogenes.py
    pangenome_screening = pd.read_csv(pangenome_screening_file, sep='\t')
    # select the columns 'Pater' and 'Attributes
    untangle_attributes = pangenome_screening[['Pater', 'Attributes']]
...
def exonerate_results_tair(exonerate_results_tair_file):
    ...
    #rename columns
    exonerate_results_tair.columns = ['Assembly', 'Path', 'Path_length', 'Path_start', 'Path_end', 'Path_strand', 'Pater', 'Feature_length', 'Feature_start', 'Feature_end', 'Alignment_length', 'Estimated_identity', 'Coverage', 'Classification', 'Attributes']
    # keep only rows where Classification is 'pseudogene'
    exonerate_tair_pseudogenes = exonerate_results_tair[exonerate_results_tair['Classification'] == 'pseudogene']
    # drop column 'Classification'
    exonerate_tair_pseudogenes = exonerate_tair_pseudogenes.drop(columns=['Classification'])
    # add column called 'Source' to the dataframe and set all values to 'exonerate' 
    exonerate_tair_pseudogenes['Source'] = 'exonerate'
    # create a new df 'exonerate_tair_genes_attributes' by selecting the columns 'Pater' and 'Attributes'
    exonerate_tair_pseudogenes_attributes = exonerate_tair_pseudogenes[['Pater', 'Attributes']]
    # drop column 'Attributes'
    exonerate_tair_pseudogenes = exonerate_tair_pseudogenes.drop(columns=['Attributes'])
    return exonerate_tair_pseudogenes, exonerate_tair_pseudogenes_attributes
...
def exonerate_results_new(exonerate_results_new_file):
    ...
    #rename columns
    exonerate_results_new.columns = ['Assembly', 'Path', 'Path_length', 'Path_start', 'Path_end', 'Path_strand', 'Pater', 'Feature_length', 'Feature_start', 'Feature_end', 'Alignment_length', 'Estimated_identity', 'Coverage', 'Classification', 'Attributes']
    # keep only rows where Classification is 'pseudogene'
    exonerate_new_pseudogenes = exonerate_results_new[exonerate_results_new['Classification'] == 'pseudogene']
    # drop column 'Classification'
    exonerate_new_pseudogenes = exonerate_new_pseudogenes.drop(columns=['Classification'])
    # add column called 'Source' to the dataframe and set all values to 'exonerate_results_new' 
    exonerate_new_pseudogenes['Source'] = 'exonerate'
    # create a new df 'exonerate_new_genes_attributes' by selecting the columns 'Pater' and 'Attributes'
    exonerate_new_pseudogenes_attributes = exonerate_new_pseudogenes[['Pater', 'Attributes']]
    # drop column 'Attributes'
    exonerate_new_pseudogenes = exonerate_new_pseudogenes.drop(columns=['Attributes'])
    return exonerate_new_pseudogenes, exonerate_new_pseudogenes_attributes

def merge_pafs_and_count(untangle_paf, exonerate_tair_pseudogenes, exonerate_new_pseudogenes, number_of_assemblies, softcore_limit):
    logging.info('Merging PAF files and counting...')
    # merge the untangle_paf with the exonerate_tair_genes and exonerate_new_genes
    all_data = pd.concat([untangle_paf, exonerate_tair_pseudogenes, exonerate_new_pseudogenes])
    # Group by 'Feature' and 'Assembly', then count unique 'Assembly' for each 'Pater'
    counts_total = all_data.groupby(['Pater', 'Assembly']).size().reset_index().groupby('Pater').size()
    ...
    counts_x_assembly = (all_data.groupby(['Pater', 'Assembly', 'Estimated_identity', 'Coverage', 'Source'])
            .size()
...
def merge_attributes(untangle_attributes, exonerate_tair_pseudogenes_attributes, exonerate_new_pseudogenes_attributes):
    ...
    all_attributes = pd.concat([untangle_attributes, exonerate_tair_pseudogenes_attributes, exonerate_new_pseudogenes_attributes])
    
    # Group by 'Pater' and merge 'Attributes'
    all_attributes['Attributes'] = all_attributes['Attributes'].fillna('')
    all_attributes = all_attributes.groupby('Pater')['Attributes'].apply(lambda x: ', '.join(set(', '.join(x).split(', ')))).reset_index()
...
def merge_paf_and_attributes(counts, all_attributes):
    ...
    all_data_final = pd.merge(counts, all_attributes, on='Pater')
    # Save the DataFrame to a CSV file
    all_data_final.to_csv('final_pseudogenes_screening.csv', sep='\t', index=False)
...
def get_pav_matrix(counts):
    ...
    presence_absence_matrix = pd.DataFrame(0, index=counts['Pater'], columns=all_assemblies)

    # Step 3: Populate the matrix
    for index, row in counts.iterrows():
        assemblies = row['Assembly_names'].split(', ')
        presence_absence_matrix.loc[row['Pater'], assemblies] = 1
...
def main():
    ...
    exonerate_tair_pseudogenes, exonerate_tair_pseudogenes_attributes = exonerate_results_tair(args.exonerate_results_tair_file)
    exonerate_new_pseudogenes, exonerate_new_pseudogenes_attributes = exonerate_results_new(args.exonerate_results_new_file)
    
    counts = merge_pafs_and_count(untangle_paf_data, exonerate_tair_pseudogenes, exonerate_new_pseudogenes, args.number_of_assemblies, args.softcore_limit)
    all_attributes = merge_attributes(untangle_attributes, exonerate_tair_pseudogenes_attributes, exonerate_new_pseudogenes_attributes)
